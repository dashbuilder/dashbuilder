...
public abstract class ConstraintDef<A extends Annotation> {
	...
	protected ConstraintDef(Class<?> beanType, Class<A> constraintType, String property, ElementType elementType, Map<String, Object> parameters, ConstraintMapping mapping) {
		if ( beanType == null ) {
			throw new RuntimeException( "Null is not a valid bean type" );
		}

		if ( mapping == null ) {
			throw new RuntimeException( "ConstraintMapping cannot be null" );
		}

		if ( ElementType.FIELD.equals( elementType ) || ElementType.METHOD.equals( elementType ) ) {
			if ( property == null || property.length() == 0 ) {
				throw new RuntimeException( "A property level constraint cannot have a null or empty property name" );
			}
