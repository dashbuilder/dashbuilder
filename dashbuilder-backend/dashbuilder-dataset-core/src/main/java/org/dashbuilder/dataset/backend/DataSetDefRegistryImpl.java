...
public class DataSetDefRegistryImpl implements DataSetDefRegistry {
    ...
    public synchronized void registerDataSetDefPreProcessor(String dataSetUUID, DataSetPreprocessor preprocessor) {
        DataSetDefEntry record = dataSetDefMap.get(dataSetUUID);
        if(record == null){
            throw new IllegalStateException("DataSetDef not found: " + dataSetUUID);
        }
